...
open FsResponses
open FsOpCore
open FsOpCore.FlUtils

module TaskFlow_PortPin =

    type SubState = {
        task                : TaskState<TMOnboard.Flow.TaskFlowMsgIn,TMOnboard.Flow.TaskFlowMsgOut>
        ...
        with 
            ...
            static member hookTaskTools (ss:SubState) =
                ...
                tasktools.SetFunctions({Functions.TaskToolImpl.taskDone = fun ()->async{                    
                    ...
                    ss.task.bus.PostToFlow (W_Msg TMOnboard.Flow.APi_TerminateTask)}}) //wire task tools plugin to this instance
                // let billf = ss.task.kernel.GetRequiredService<BillFunctions>()
                // billf.SetBus ss.task.bus //wire bill functions plugin to this instance
    ...
    module States = 
        ...
        let cuaLoopRequest (ss:SubState) cc = 
            let instr =                
                ...
                |> Prompts.renderPrompt Cua_Prompts.``cua loop``
            {CuaReq.Default with
        ...
        let cuaStartRequest (ss:SubState) = 
            ...
            let devPrompt = [Vars.cuaInstructions, ss.task.cuaPrompt :> obj] |> Prompts.renderPrompt Cua_Prompts.``cua step start``
        ...
        and s_cua ss msg = async {
            ...
            | M (CUAi_ComputerCall cc) when ss.cuaLoopCount > C.MAX_CUA_CALLS_IN_TASK ->                                //cua is stuck in a loop
                                               ...
                                               let msgs = 
                                                    ss.task.visualState 
                                                    |> Option.map(fun ss -> [APo_Screenshot ss.snapshot])
                                                    |> Option.defaultValue []
                                               let ss = {ss with task=task}.incrCuaLoopCount()                          //incr cua loop count
                                               let req = cuaStartRequest ss
                                               return F(s_cua ss, CUAo_Req req::msgs )
            | M (CUAi_ComputerCall cc)      -> let! task = ss.task.performActionAndCapture cc                           //handle computer call
                                               ...
                                               return F(s_cua ss,[CUAo_Req req; APo_Action (Actions.actionToString cc.action)] @ msgs) //txn back to s_cua; send action to app and loop
