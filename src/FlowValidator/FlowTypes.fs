...
type ElemRef =
    ...
    with 
        ...
        member this.Desc with get() = if this.elementId.IsSome then $"{this.elementId.Value}"
                                      elif this.path.IsSome then $"{this.path.Value}"
                                      elif this.xpath.IsSome then $"{this.xpath.Value}"
                                      else $"{this.aria_label},{this.css_classes},{this.inner_text}"
...
type FlowStep =
    ...
    | Pause of string
    with
        member this.Desc with get() = match this with Clicks (x::_) -> $"Click {x.Desc}" | x -> string x 
