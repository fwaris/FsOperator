...
module TaskFlowDynamic_ =
    ...
    type SubState = {
        ...
        with
            ...
            member this.appendUsage = function
                | W_Msg (TFi_Usage (id,usg)) -> {this with task = this.task.appendUsage (id,usg)}
                | _ -> this
            ...
            member this.delayPostGetSteps() =     
                async {
                    ...
                    this.task.bus.PostToFlow (W_App TFi_GetSteps)
                }
                |> Async.Start
            ...
            static member hookTaskTools (ss:SubState) =
                ...
                tasktools.SetFunctions({Functions.TaskToolImpl.taskDone = fun ()->async{                    
                    ...
                    ss.task.bus.PostToFlow (W_App TaskFlowMsgIn.TFi_TerminateTask)}}) //wire task tools plugin to this instance
    ...
    module States =
        ...
        let rec (|Txn|Cont|)  (s_ret,ss:SubState,msg) =
            ...
            | W_Msg TFi_EndAndReport         -> Txn (async {
            ...
            | W_Msg TFi_TerminateTask        -> Txn (async {
                                                        let ss = ss.appendUsage msg
        ...
        and s_start ss msg = async {
            ...
            | Txn st                         -> return! st
            | Cont (W_Msg (TFi_App(TFi_Start)),ssa) -> 
                                                let! ss,ms = ssa ss
        ...
        and s_pause (ss:SubState) msg = async {
            ...
            match s_pause,ss,msg with
            | Txn st                         -> return! st
            | Cont (W_Msg TFi_GetSteps,ssa)  -> let! ss,ms = ssa ss
                                                let ss = ss.tryPostUpdateSteps()
                                                ss.delayPostGetSteps()
                                                return F(s_pause ss,ms)
            | Cont (GotSteps ss ss',ssa)     -> let! ss',ms = ssa ss'
