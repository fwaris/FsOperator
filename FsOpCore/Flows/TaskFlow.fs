...
//BE BRIEF
    ...
    let postResumeCua ss snapshot =
        async {
            ...
            let req = {CuaReq.Default with instructions=ss.chat.systemMessage; snapshot=snapshot; chatHistory=chatHistory}
            FlResps.postStartCua ss.bus.PostInput req
        }
        |> FlResps.catch ss.bus.PostInput
    ...
    let rec s_start ss msg = async {
        ...
        | W_App TFi_Start -> let! (snapshot,w,h,url,env) as sn = snapshot ss.driver
                             let ss = ss.appendSnapshot snapshot
                             let req = {CuaReq.Default with instructions=ss.chat.systemMessage; snapshot=sn}
                             FlResps.postStartCua ss.bus.PostInput req
                             return !!(s_loop ss)
